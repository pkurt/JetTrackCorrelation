...
float TrkPtBins[nTrkPtBins+1] = {1, 2, 3, 5, 999};
TString TrkPtBin_strs[nTrkPtBins+1] = {"TrkPt1","TrkPt2", "TrkPt3", "TrkPt5", "TrkPt999" };
...
hist_class::hist_class(TString the_desc, bool is_it_data) {
  ...
  for (int ibin=0;ibin<nCBins;ibin++){
	...
	/*
	   hJetTrackSignalBackground[ibin][ibin2][ibin3] = new TH2D((TString) (desc + "_hJetTrackSignalBackground"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+ "_" + TrkPtBin_strs[ibin3] + "_" + TrkPtBin_strs[ibin3+1]), "", 101,-5,5,75,-TMath::Pi(),2*TMath::Pi());     hJetTrackSignalBackground[ibin][ibin2][ibin3]->Sumw2();

	   hJetTrackBackground[ibin][ibin2][ibin3] = new TH2D((TString) (desc + "_hJetTrackBackground"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+ "_" + TrkPtBin_strs[ibin3] + "_" + TrkPtBin_strs[ibin3+1]), "", 101,-5,5,75,-TMath::Pi(),2*TMath::Pi());  hJetTrackBackground[ibin][ibin2][ibin3]->Sumw2();
	...
	TrkEta[ibin][ibin2][ibin3] = new TH1F((TString) (desc + "_TrkEta"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+ "_" + TrkPtBin_strs[ibin3] + "_" + TrkPtBin_strs[ibin3+1]), "", 50, -5., 5.);     TrkEta[ibin][ibin2][ibin3]->Sumw2();
...
//          for(int evi = 0; evi < 10000; evi++) {
	...
	my_hists->track_vz_weighted->Fill(vz, wvz*wcen);
      ...
      ////// EMix for gen-tracks and recjets/////////////
      ////// EMix for gen-tracks and recjets/////////////
      ////// EMix for gen-tracks and recjets/////////////
	...
	double thisjeteta = my_primary->jteta->at(j4i);
	double thisjetphi = my_primary->jtphi->at(j4i);
	double thiscent = my_primary->hiBin;

	//! loop through the next n events to pair with this event

	int mixentry = evi;
	int n_evts_I_have_used = 0;
	//    for(int k = 0 ; k < 20 ; ++k)
	while(1)
	{

	  //	      if( evi % 1 == 0 ) std::cout << "evi1: " << evi << ",  vz1 : " << vz << ", mb_vz1: " << my_primary->vz->at(0)  << "\n";


	  //if( n_evts_I_have_used >= 5 ) break;
          if( n_evts_I_have_used >= 20 ) break;
	  ...
	  //	      if( evi % 1 == 0 ) std::cout << "evi2: " << evi << ",  vz2 : " << vz << ", mb_vz2: " << my_primary->vz->at(0)  << "\n";

	  if( mixentry == evi ) break;
	  my_primary->GetEntry(mixentry);

	  //	      if( evi % 1 == 0 ) std::cout << "evi3: " << evi << ",  vz3 : " << vz << ", mb_vz3: " << my_primary->vz->at(0)  << "\n";

	  /// event matching
//	  if( fabs(vz - my_primary->vz->at(0)) > 5.0 ) continue;
//	  if( fabs(thiscent-my_primary->hiBin) / thiscent > 0.25 ) continue;

	  if( fabs(vz - my_primary->vz->at(0)) > 2.0 ) continue;
	  if( fabs(thiscent-my_primary->hiBin) / thiscent > 0.1 ) continue;




	  //	      if( evi % 1 == 0 ) std::cout << "evi5: " << evi << ",  vz5 : " << vz << ", mb_vz5: " << my_primary->vz->at(0)  << " ,  cent : " << thiscent << " , min_cetn:  " << my_primary->hiBin  << ", mb evt: " <<mixentry << " of " << n_evts_I_have_used << "\n";
	  ...
	  if( !is_data ) {
	    ...
	    for(int ipart = 0 ; ipart < (int) my_primary->pPt->size(); ipart++){ //sim track loop 
	      ...
	      //	double rr = mydeltaR(my_ct->jteta[j4i],my_ct->jtphi[j4i], gen_eta,gen_phi);
...
//                std::cout << " dEta : " << deta_bkg << "  dPhi : " << dphi_bkg << std::endl;
	...
	//if( fabs(my_primary->jteta->at(j4i)) > 2 ) continue;
	//if(my_primary->trackMax->at(j4i)/my_primary->jtpt->at(j4i) <=0.01) continue ;            
	//if( my_primary->jtpt->at(j4i) <100 ) continue;
	//if( my_primary->jtpt->at(j4i) >300 ) continue;

//	std::cout << "WHY IS JET pT=0 : " << my_primary->jtpt->at(j4i) << std::endl;
	...
	my_hists->Centrality_hist[ibin][ibin2]->Fill(my_primary->hiBin, wvz*wcen);
	...
	int trk_multiplicity_gen_all =0;
	int trk_multiplicity_gen =0;
	int trk_multiplicity_gen_bkg =0;
	int trk_multiplicity_gen_all_bkg =0;

	std::vector<double> MultiplicityRBin_gen(NRBIN_gen);
	std::vector<double> MultiplicityRBin_gen_bkg(NRBIN_gen);
	std::vector<double> MultiplicityRBin_gen_all(NRBIN_gen);
	std::vector<double> MultiplicityRBin_gen_all_bkg(NRBIN_gen);
	...
	if( !is_data ) {
	  ...
	  for(int ipart = 0 ; ipart < (int) my_primary->pPt->size() ; ipart++){ //sim track loop 
		...
		if (gen_pt >=TrkPtBins[trkpti] && gen_pt < TrkPtBins[trkpti+1])  ibin3 = trkpti ;
	      ...
	      if(rr<=0.3){
		my_hists->Multiplicity_vs_radius_gen[ibin][ibin2]->Fill(rr, wvz*wcen);
		my_hists->Multiplicity_vs_radius_gen_ptwt[ibin][ibin2]->Fill(rr, gen_pt*wvz*wcen);
	      }
	    //}
		...
		if(RBIN_gen[ir]<rr && rr <= RBIN_gen[ir+1]){
		  trk_multiplicity_gen_all++;
		  MultiplicityRBin_gen_all[ir]= trk_multiplicity_gen_all*gen_pt;
		}
	      }
	    }		
	  ...
	  for(int ipart = 0 ; ipart < (int) my_primary->pPt->size(); ipart++){ //sim track loop 
	    ...
	    if(TMath::Abs(gen_eta)>2.4)continue ;
	    double rr = mydeltaR(my_primary->jteta->at(j4i),my_primary->jtphi->at(j4i), gen_eta,gen_phi);
	    /// commmentout this because fig65 where it should use all charged-gen particles around the RecJetexis
	    //if(my_primary->pNRec->at(ipart)>0&&(my_primary->mtrkAlgo->at(ipart)<4||(my_primary->mtrkQual->at(ipart)))){
	    if(rr<=0.3){
	      for(int ir=0;ir<NRBIN_gen;ir++){
		if(RBIN_gen[ir]<rr && rr <= RBIN_gen[ir+1]){

		  trk_multiplicity_gen++;
		  MultiplicityRBin_gen[ir]= trk_multiplicity_gen*gen_pt;
		  //MultiplicityRBin_gen[ir]= trk_multiplicity_gen;
	  ...
	  if( fabs(my_primary->jteta->at(j4i)) > 0.3 ){
		...
		for(int ir=0;ir<NRBIN_gen;ir++){
		  if(RBIN_gen[ir]<rr && rr <= RBIN_gen[ir+1]){
		    trk_multiplicity_gen_all_bkg++;
		    MultiplicityRBin_gen_all_bkg[ir]= trk_multiplicity_gen_all_bkg*gen_pt;
		  }
		}
	      }
	      ...
	      if(my_primary->pNRec->at(ipart)>0&&(my_primary->mtrkAlgo->at(ipart)<4||(my_primary->mtrkQual->at(ipart)))){
		if(rr<0.3){
		  my_hists->Multiplicity_vs_radius_gen_bkg_ptwt[ibin][ibin2]->Fill(rr, gen_pt*wvz*wcen);
		  my_hists->Multiplicity_vs_radius_gen_bkg[ibin][ibin2]->Fill(rr, wvz*wcen);

		}
	      }  // this mtaching for only dR plot where I wanted to check dR for sim-matched tracks

	      for(int ir=0;ir<NRBIN_gen;ir++){
		if(RBIN_gen[ir]<rr && rr <= RBIN_gen[ir+1]){

		  trk_multiplicity_gen_bkg++;
		  MultiplicityRBin_gen_bkg[ir]= trk_multiplicity_gen_bkg*gen_pt;
		  //MultiplicityRBin_gen_bkg[ir]= trk_multiplicity_gen_bkg;
	...
	int trk_multiplicity =0;
	int trk_multiplicity_bkg =0;
	int ntracks = 0;
	...
	float trkweight = 1.;
	float trkweight_num = 1.;
	float trkweight_bkg_er = 1.;
	float trkweight_bkg_er_test = 1.;
	...
	for(int tracks =0; tracks < (int) my_primary->trkPt->size(); tracks++){
	  ...
	  if (my_primary->trkAlgo->at(tracks)<4 || my_primary->highPurity->at(tracks)==1) {
	    if(my_primary->trkPt->at(tracks)>trkPtCut) {
	      //  		   if(my_primary->trkPt->at(tracks)<3) continue;
	      //                 if(my_primary->trkPt->at(tracks)>5) continue;

	      ntracks++;



//	      std::cout << "my_primary->jtpt->at(j4i): " << my_primary->jtpt->at(j4i) << ", trkPt: " << my_primary->trkPt->at(tracks) << "\n";
	      ...
	      double Yaxian_rr =TMath::Sqrt((my_primary->trkPhi->at(tracks)-my_primary->jtphi->at(j4i))*(my_primary->trkPhi->at(tracks)-my_primary->jtphi->at(j4i))+(my_primary->trkEta->at(tracks)-my_primary->jteta->at(j4i))*(my_primary->trkEta->at(tracks)-my_primary->jteta->at(j4i)));

	      // std::cout << " mydeltaR mine " << rr << " TVector3:  " << delr << "  yaxian's :  " <<  Yaxian_rr << std::endl;
	      //          std::cout << " jetEta " <<  my_ct->jteta[j4i] << "  jet phi: " <<  my_ct->jtphi[j4i] << " trkEta : " << my_primary->trkEta->at(tracks)  << " trkPhi: " << my_primary->trkPhi->at(tracks) <<   "   TVector3:  " << delr << "  yaxian's :  " <<  Yaxian_rr << std::endl;


	      double ptParticle=my_primary->trkPt->at(tracks);
	      float dphi_jet_track = my_primary->jtphi->at(j4i) - my_primary->trkPhi->at(tracks);

	      my_hists->dPhi_jet_track[ibin][ibin2]->Fill(fabs(dphi_jet_track), wvz*wcen);


	      if(Yaxian_rr<0.3){ 
		my_hists->radius_hist[ibin][ibin2]->Fill(Yaxian_rr, wvz*wcen);
		//my_hists->track_cand_pT_hist[ibin][ibin2]->Fill(my_primary->trkPt->at(tracks), wvz*wcen);			       
	      ...
	      my_hists->TrkPt[ibin][ibin2][ibin3]->Fill(my_primary->trkPt->at(tracks),trkweight*wvz*wcen);
	      my_hists->TrkEta[ibin][ibin2][ibin3]->Fill(my_primary->trkEta->at(tracks),trkweight*wvz*wcen);
	      my_hists->TrkPhi[ibin][ibin2][ibin3]->Fill(my_primary->trkPhi->at(tracks),trkweight*wvz*wcen);
		...
		my_hists->radius_hist_mine[ibin][ibin2]->Fill(rr, wvz*wcen);
		my_hists->radius_hist_mine2[ibin][ibin2]->Fill(rr, wvz*wcen*ptParticle);
		my_hists->radius_hist_mine3[ibin][ibin2]->Fill(rr, (wvz*wcen*ptParticle)/my_primary->jtpt->at(j4i));

		//if (evi%1==0) std::cout << ", evi: " << evi << " of " << n_evt << "jet index : " << j4i << my_ct->jtpt[j4i] << " jet eta : "  << my_ct->jteta[j4i] <<  "   jetphi :  " << my_ct->jtphi[j4i] <<    "   trkId : " << tracks <<  "   trkpt : " << my_primary->trkPt->at(tracks) << "  trkEta: " << my_primary->trkEta->at(tracks) << "   trkPhi : " <<  my_primary->trkPhi->at(tracks) << "    mydeltaR " << rr  <<  std::endl;

		//	       if( doTrackCorrections ) trkweight = trackCorrections[0]->GetCorr(my_primary->trkPt->at(tracks),my_primary->trkEta->at(tracks),my_ct->jtpt[j4i],my_ct3->hiBin);

		//if(doTrackCorrections) trkweight = c->getTrackCorrection(tracks);

		//       std::cout <<"cent : " << ibin <<  "   trkweight : " << trkweight << std::endl;

		//			my_hists->track_cand_pT_hist_weighted[ibin][ibin2]->Fill(my_primary->trkPt[tracks], trkweight*wvz*wcen);
		//my_hists->track_cand_pT_hist[ibin][ibin2]->Fill(my_ct5->trkPt[tracks], wvz*wcen);

		my_hists->Multiplicity_vs_radius_ptwt[ibin][ibin2]->Fill(rr, ptParticle*trkweight*wvz*wcen);
		my_hists->Multiplicity_vs_radius[ibin][ibin2]->Fill(rr, trkweight*wvz*wcen);
	...
	my_hists->Ntracks[ibin][ibin2][ibin3]->Fill(ntracks, wvz*wcen);
	my_hists->NumChargedParticles[ibin][ibin2]->Fill(ntracks, wvz*wcen);
	...
	std::vector<double> MultiplicityRBin(NRBIN);
	std::vector<double> MultiplicityRBin_bkg(NRBIN);

	for(int tracks =0; tracks < (int) my_primary->trkEta->size(); tracks++){
	  if (my_primary->trkAlgo->at(tracks)<4 || my_primary->highPurity->at(tracks)==1) {
	    if(fabs(my_primary->trkEta->at(tracks))>2.4) continue;
	    if(my_primary->trkPt->at(tracks)>trkPtCut) {
	      //		     if(my_primary->trkPt->at(tracks)<3) continue;
		...
		if(RBIN[ir]<rr && rr <= RBIN[ir+1]){
		  ...
		  trk_multiplicity++;
		  MultiplicityRBin[ir]= trk_multiplicity*ptParticle*trkweight_num;
		  //MultiplicityRBin[ir]= trk_multiplicity*trkweight_num;
	...
	if( fabs(my_primary->jteta->at(j4i)) > 0.3 ){
	  for(int tracks =0; tracks < (int) my_primary->trkEta->size(); tracks++){
		...
		double rr = mydeltaR(-(my_primary->jteta->at(j4i)),my_primary->jtphi->at(j4i), my_primary->trkEta->at(tracks),my_primary->trkPhi->at(tracks));
		ptParticle_er=my_primary->trkPt->at(tracks);

		for(int ir=0;ir<NRBIN;ir++){
		  if(RBIN[ir]<rr && rr <= RBIN[ir+1]){

		    Float_t trkweight_bkg_er = my_primary->trkweight->at(tracks);

		    my_hists->track_bkg_pT_hist[ibin][ibin2]->Fill(my_primary->trkPt->at(tracks), wvz*wcen);			    
		    my_hists->track_bkg_pT_hist_weighted[ibin][ibin2]->Fill(my_primary->trkPt->at(tracks), trkweight_bkg_er*wvz*wcen);


		    if(rr<0.3){
		      my_hists->Multiplicity_vs_radius_bkg[ibin][ibin2]->Fill(rr, trkweight_bkg_er*wvz*wcen);
		      my_hists->Multiplicity_vs_radius_bkg_ptwt[ibin][ibin2]->Fill(rr, ptParticle_er*trkweight_bkg_er*wvz*wcen);

		    }



		    trk_multiplicity_bkg++;
		    MultiplicityRBin_bkg[ir]= trk_multiplicity_bkg*ptParticle_er*trkweight_bkg_er;	
		    //MultiplicityRBin_bkg[ir]= trk_multiplicity_bkg*trkweight_bkg_er;          
	...
	for(int ir=0;ir<NRBIN;ir++){
	  if(my_primary->jtpt->at(j4i)>0.0001){
	    ...
	    if( !is_data ) {
	      DifferentialPtGen[ir] =SumPtRBinGen[ir]/(my_primary->jtpt->at(j4i));
	    }
	  }

	  if( fabs(my_primary->jteta->at(j4i)) > 0.3 ){
	    if(my_primary->jtpt->at(j4i)>0.0001){
	      DifferentialPtEtaRef[ir] =SumPtRBinEtaRef[ir]/(my_primary->jtpt->at(j4i));
	      if( !is_data ) {
		DifferentialPtEtaRefGen[ir] =SumPtRBinEtaRefGen[ir]/(my_primary->jtpt->at(j4i));
	      }
	    }
	  }
	}
	...
	for(int ir=0;ir<NRBIN;ir++){
	  ...
	  if(my_primary->jtpt->at(j4i)>0.0001){
	    ...
	    if(ir==5){
	      my_hists->JetShapeDiffParticles_1D_check_error[ibin][ibin2]->Fill(DifferentialPt[ir]*wvz*wcen);
	    }
	...
	for(int ir=0;ir<NRBIN;ir++){
	  ...
	  if(my_primary->jtpt->at(j4i)>0.0001){
	    my_hists->JetShapeIntegratedParticles_1D[ibin][ibin2]->Fill(fill_val,(tot_sumpt_rbin[ir]/my_primary->jtpt->at(j4i))*wvz*wcen);
	  }
  ...
  /*
     ...
     if(my_ct->trackMax[j4i] <=4) continue ;
     ...
     lead_pt=jet_pt;
     highest_idx=j4i;
     highest_jet_vec.SetPtEtaPhi(jet_pt, my_ct->jteta[j4i], my_ct->jtphi[j4i]);
  ...
  double jet_phi =my_ct->jtphi[highest_idx];
  if(jet_phi<=-TMath::Pi())jet_phi+=TMath::TwoPi();
  double jet_eta = my_ct->jteta[highest_idx];
  ...
  if(my_ct->trackMax[ijet] <=4) continue ;
  if(my_ct->rawpt[ijet] <= 15 ) continue ;
  if(my_ct->jtpt[ijet]<50) continue ;


  if(my_ct->jtpt[ijet] > sublead_pt){
  sublead_pt=my_ct->jtpt[ijet];
  second_highest_idx=ijet;
  second_highest_jet_vec.SetPtEtaPhi(my_ct->jtpt[ijet], my_ct->jteta[ijet], my_ct->jtphi[ijet]);
  ...
  //  if(fabs(my_ct->jteta[highest_idx])>2 || fabs(my_ct->jteta[second_highest_idx])>2) continue;
  if(my_ct->jtpt[highest_idx]< 100. ) continue;
  if( my_ct->jtpt[second_highest_idx]<50) continue;
  ...
  for(int ji = 0; ji < my_ct->nref ; ji++ ) {
    if( fabs(my_ct->jteta[ji]) > 3 ) continue;
    if( ji == highest_idx ) continue;
    if( ji == second_highest_idx ) continue;
    if(my_ct->trackMax[ji] <=4) continue ;
    if(my_ct->rawpt[ji] <= 15 ) continue ;
    float pT = my_ct->jtpt[ji];
    if( pT > my_ct->jtpt[highest_idx] || pT > my_ct->jtpt[second_highest_idx]) {
      std::cout << "Error, sorting jets by pT did not work. Highest pT: " << my_ct->jtpt[highest_idx]  << ", idx: " << highest_idx << ", second highest: " << my_ct->jtpt[second_highest_idx] << ", idx: " << second_highest_idx << ", but idx " << ji << " has pT:"  << pT << std::endl;
      assert(0);
    }
  ...
  //		    if (dphi>TMath::Pi()) dphi = 2*TMath::Pi() - dphi;
  // if(fabs(dphi)<=(2*TMath::Pi())/3) continue;

  double subjet_phi =my_ct->jtphi[second_highest_idx];
  if(subjet_phi<=-TMath::Pi())subjet_phi+=TMath::TwoPi();
  double subjet_eta = my_ct->jteta[second_highest_idx];
  double dphi = subjet_phi-jet_phi;
  double deta = subjet_eta-jet_eta;


  for (int ibin=0;ibin<nCBins;ibin++){
    for (int ibin2=0;ibin2<nPtBins;ibin2++){
      if (my_ct3->hiBin >=CBins[ibin] && my_ct3->hiBin<CBins[ibin+1]){
	if (my_ct->jtpt[highest_idx] >=PtBins[ibin2] && my_ct->jtpt[highest_idx] < PtBins[ibin2+1]){
	  my_hists->dPhi_hist[ibin][ibin2]->Fill(fabs(dphi));
	  my_hists->dEta_hist[ibin][ibin2]->Fill(deta);

	  my_hists->LeadingJetPhi_hist[ibin]->Fill(jet_phi);
	  my_hists->LeadingJetEta_hist[ibin]->Fill(jet_eta);
	  my_hists->LeadingJetPt_hist[ibin]->Fill(my_ct->jtpt[highest_idx]);


	  my_hists->SubJetPhi_hist[ibin]->Fill(subjet_phi);
	  my_hists->SubJetEta_hist[ibin]->Fill(subjet_eta);
	  my_hists->SubJetPt_hist[ibin]->Fill(my_ct->jtpt[second_highest_idx]);
  ...
  //std::cout << " ievt= " <<  evi<< ", idx_leading: " << highest_idx <<  ",  pT= " << my_ct->jtpt[highest_idx]  << ",  eta= " << my_ct->jteta[highest_idx] << " phi= : " << my_ct->jtphi[highest_idx] << " !!!!! idx_subleading: " << second_highest_idx << ", sub pT: " << my_ct->jtpt[second_highest_idx] <<  ", sub eta : " << my_ct->jteta[second_highest_idx] << ", sub phi : " << my_ct->jtphi[second_highest_idx] << std::endl;
