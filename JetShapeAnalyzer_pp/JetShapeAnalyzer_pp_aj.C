...
void StudyFiles(std::vector<TString> file_names, int foi, hist_class *my_hists)
{
  ...
  TH1F* hWeight_pt[nCBins];
  TH1F* hWeight_pp_pt;

  TFile * wtfile_pt = TFile::Open("pbpbData_jetpt_inc120_official_data.root", "readonly");
  TFile * wtfile_pp_pt = TFile::Open("ppdata_jetpt_inc120.root", "readonly");


  if(is_dijet){

    if(is_leadingjet){
      wtfile_pt = TFile::Open("pbpb_jetpt_lead120_official_data.root", "readonly");
      wtfile_pp_pt = TFile::Open("pp_jetpt_lead120.root", "readonly");
    }

    if(is_subleadingjet){
      wtfile_pt = TFile::Open("pbpb_jetpt_sub50_official_data.root", "readonly");
      wtfile_pp_pt = TFile::Open("pp_jetpt_sub50.root", "readonly");
    }

    hWeight_pp_pt = (TH1F*) wtfile_pp_pt->Get("hists_all_jets_corrpTCent0_Cent10_Pt100_Pt300");
    hWeight_pp_pt->Scale(1./hWeight_pp_pt->Integral());

  }else{
    wtfile_pt = TFile::Open("pbpbData_jetpt_inc120_official_data.root", "readonly");
    wtfile_pp_pt = TFile::Open("ppdata_jetpt_inc120.root", "readonly");
    hWeight_pp_pt = (TH1F*) wtfile_pp_pt->Get("hists_all_jets_corrpTCent0_Cent100_Pt100_Pt300");
    hWeight_pp_pt->Scale(1./hWeight_pp_pt->Integral());

  }

  for(int ibin = 0 ; ibin <nCBins; ibin++){
    hWeight_pt[ibin] = (TH1F*) wtfile_pt->Get("hists_all_jets_corrpT"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]+ "_Pt100_Pt300" );
    hWeight_pt[ibin]->Scale(1./hWeight_pt[ibin]->Integral());
    // wtfile_pt->Close();
  }
    ...
    for(int evi = 0; evi < n_evt; evi++) {
      ...
      for(int j4i = 0; j4i < (int) my_primary->jtpt->size(); j4i++) {
        ...
        for(int ibin = 0; ibin < nCBins; ibin++) {     /// centrality bin loop
          ...
          int pt_bin = hWeight_pt[ibin]->GetXaxis()->FindBin(my_primary->jtpt->at(j4i));
          float num_pt = hWeight_pt[ibin]->GetBinContent(pt_bin);  /// PbPb data
          float denom_pt = hWeight_pp_pt->GetBinContent(pt_bin); /// pp data
          double pt_reweight = 1.;
          if( denom_pt > 0.0001 ) pt_reweight = num_pt / denom_pt;
      ...
      for(int ir=0;ir<NRBIN;ir++){
        ...
        if(my_primary->jtpt->at(j4i)>0.0001){
          my_hists->JetShapeDiffParticles_1D[ibin][ibin2]->Fill(fill_val,DifferentialPt[ir]*wvz*wcen*pt_reweight);
        }
      }
      ...
      while(1) {
        ...
        int ibin = 0;      int ibin2 = 0;  int ibin3=0;
        for(int ibin = 0; ibin < nCBins; ibin++) {     /// centrality loop

          for(int pti = 0; pti < nPtBins; pti++) { 
            if (thisjetpt >=PtBins[pti] && thisjetpt < PtBins[pti+1])  ibin2 = pti ;
          } /// pti loop

          my_hists->emix_jetpt[ibin][ibin2]->Fill(1, wvz*wcen*pt_reweight);
          ...
          if( !is_data ) {
            for(int ipart = 0 ; ipart < (int) my_primary->pPt->size(); ipart++){ //sim track loop 
              ...
              my_hists->hJetGenTrackBackground[ibin][ibin2][ibin3]->Fill(deta_bkg_gen,dphi_bkg_gen,wvz*wcen*pt_reweight);
              my_hists->hJetGenTrackBackground[ibin][ibin2][ibin3]->Fill(deta_bkg_gen,dphi_bkg_gen+(2*TMath::Pi()),wvz*wcen*pt_reweight);
              my_hists->hJetGenTrackBackground[ibin][ibin2][ibin3]->Fill(deta_bkg_gen,dphi_bkg_gen-(2*TMath::Pi()),wvz*wcen*pt_reweight);
          ...
          for(int tracks =0; tracks < (int) my_primary->trkPt->size(); tracks++){
            if (my_primary->highPurity->at(tracks)==1) {
              ...
              if(my_primary->trkPt->at(tracks)>trkPtCut) {
                ...
                my_hists->hJetTrackBackground[ibin][ibin2][ibin3]->Fill(deta_bkg,dphi_bkg,trkweight_bkg*wvz*wcen*pt_reweight);
                my_hists->hJetTrackBackground[ibin][ibin2][ibin3]->Fill(deta_bkg,dphi_bkg+(2*TMath::Pi()),trkweight_bkg*wvz*wcen*pt_reweight);
                my_hists->hJetTrackBackground[ibin][ibin2][ibin3]->Fill(deta_bkg,dphi_bkg-(2*TMath::Pi()),trkweight_bkg*wvz*wcen*pt_reweight);

                my_hists->hJetTrackBackground_notrkcorr[ibin][ibin2][ibin3]->Fill(deta_bkg,dphi_bkg,wvz*wcen*pt_reweight);
                my_hists->hJetTrackBackground_notrkcorr[ibin][ibin2][ibin3]->Fill(deta_bkg,dphi_bkg+(2*TMath::Pi()),wvz*wcen*pt_reweight);
                my_hists->hJetTrackBackground_notrkcorr[ibin][ibin2][ibin3]->Fill(deta_bkg,dphi_bkg-(2*TMath::Pi()),wvz*wcen*pt_reweight);
