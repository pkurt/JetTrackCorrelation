...
class hist_class {
  public:
    ...
    TH1D* track_gen_pT_hist_matched[nCBins][nPtBins];
...
///////// gen level eta-phi-pt spectras
      ...
      track_gen_pT_hist_matched[ibin][ibin2] = new TH1D((TString) (desc + "_track_gen_pT_hist_matched"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]), "", nPtBintrk,ptBintrk);   track_gen_pT_hist_matched[ibin][ibin2]->Sumw2();
...
void hist_class::AddHists(hist_class *more_hists, float wt)
{
  ...
  for (int ibin=0;ibin<nCBins;ibin++){
    ...
    for (int ibin2=0;ibin2<nPtBins;ibin2++){ 
      ...
      track_gen_pT_hist_matched[ibin][ibin2]->Sumw2();   more_hists->track_gen_pT_hist_matched[ibin][ibin2]->Sumw2();
      track_gen_pT_hist_matched[ibin][ibin2]->Add(more_hists->track_gen_pT_hist_matched[ibin][ibin2], wt);
...
void hist_class::Delete()
{
  ...
  for (int ibin=0;ibin<nCBins;ibin++){
    ...
    for (int ibin2=0;ibin2<nPtBins;ibin2++){
      ...
      delete track_gen_pT_hist_matched[ibin][ibin2];
...
void hist_class::Write()
{
  ...
  for (int ibin=0;ibin<nCBins;ibin++){
    ...
    for (int ibin2=0;ibin2<nPtBins;ibin2++){
      ...
      track_gen_pT_hist_matched[ibin][ibin2]->Write();
...
void StudyFiles(std::vector<TString> file_names, int foi, hist_class *my_hists)
{
  ...
  TFile * wtfile_vtx = TFile::Open("pp_data_vertex.root", "readonly");
  hWeight_vtx = (TH1F*)wtfile_vtx->Get("hists_track_vz");
  hWeight_vtx->Scale(1./hWeight_vtx->Integral());
  wtfile_vtx->Close();

  TFile * wtfile_vtx_mc = TFile::Open("pythia80_vertex.root", "readonly");
  hWeight_MC_vtx = (TH1F*)wtfile_vtx_mc->Get("hists_track_vz");
...
//        if (pthat_new > pthatmax) continue;
      ...
      int vtx_bin = hWeight_vtx->GetXaxis()->FindBin(vertex);
...
//      double wvz=1;
      ...
      if(is_data) { 
        ...
        //if (my_primary->HLT_Jet60_v1==0) continue;
...
//    if( my_primary->jtpt->at(j4i) <30 ) continue;
...
//    if( fabs(my_primary->jteta->at(j4i)) < 0.3 ) continue;
  ...
  //// genparticles
	  ...
	  /// closure with gen particles instead of simTracks ///
         ...
         if( !is_data ) {
           ...
           for(int ipart = 0 ; ipart < (int) my_primary->pt->size(); ipart++){ //sim track loop 
             ...
             my_hists->track_gen_pT_hist_matched[ibin][ibin2]->Fill(gen_pt, wvz*wcen);

             
             if(sube==0) {   // selects pythia
               my_hists->hJetGenTrackSignalBackground_pythia[ibin][ibin2][ibin3]->Fill(deta_gen,dphi_gen, wvz*wcen);
               my_hists->hJetGenTrackSignalBackground_pythia[ibin][ibin2][ibin3]->Fill(deta_gen,dphi_gen+ (2.*TMath::Pi()), wvz*wcen);
               my_hists->hJetGenTrackSignalBackground_pythia[ibin][ibin2][ibin3]->Fill(deta_gen,dphi_gen- (2.*TMath::Pi()), wvz*wcen);

             }else{    // selects hydjet part
               my_hists->hJetGenTrackSignalBackground_hydjet[ibin][ibin2][ibin3]->Fill(deta_gen,dphi_gen, wvz*wcen);
               my_hists->hJetGenTrackSignalBackground_hydjet[ibin][ibin2][ibin3]->Fill(deta_gen,dphi_gen+ (2.*TMath::Pi()), wvz*wcen);
               my_hists->hJetGenTrackSignalBackground_hydjet[ibin][ibin2][ibin3]->Fill(deta_gen,dphi_gen- (2.*TMath::Pi()), wvz*wcen);
      ...
      double matched_gen_pt = my_primary->mpPt->at(ipart);
...
// if(my_primary->highPurity->at(ipart)==0 || my_primary->mtrkPtError->at(ipart)/my_primary->mpPt->at(ipart)>=0.1 || TMath::Abs(my_primary->mtrkDz1->at(ipart)/my_primary->mtrkDzError1->at(ipart))>=3.0 ||TMath::Abs(my_primary->mtrkDxy1->at(ipart)/my_primary->mtrkDxyError1->at(ipart))>=3.0) continue ;
 ...
 //     my_hists->track_gen_pT_hist_matched[ibin][ibin2]->Fill(matched_gen_pt, wvz*wcen);
...
///if(my_primary->trkPt->at(tracks)<1) std::cout<< "  trkeff : " << trkweight << "  trkpt " << my_primary->trkPt->at(tracks) << std::endl;
         ...
         my_hists->Ngen[ibin][ibin2][ibin3]->Fill(ngen, wvz*wcen);
      ...
      while(1) {
       ...
       // if( fabs(thiscent-my_primary->hiBin) / thiscent > 0.1 ) continue;
        ...
        for(int mji = 0; mji < (int) my_primary->jtpt->size(); mji++) {
          ...
          //   if( fabs(my_primary->jteta->at(mji)) < 0.3 ) continue;
        ...
        if( !is_data ) {
          ...
          for(int ipart = 0 ; ipart < (int) my_primary->pt->size(); ipart++){ //sim track loop 
            ...
            if(sube==0) {   // selects pythia
              ...
              //             std::cout << " sube_hydjet : " << sube << std::endl;
